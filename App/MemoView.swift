...
// Inlined from CoreFlow (github.com/sisoje/swift-core-flow), so this example
// needs no dependency for it: a value built from the view's inputs, kept
// across renders, rebuilt only when its `dependencies` change.
...
struct MemoView<Value, Content: View>: View {
    init(
